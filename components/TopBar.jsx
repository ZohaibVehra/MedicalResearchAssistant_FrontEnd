...
const TopBar = ({ user, onLoginClick, onLogoutClick, onMobileSidebarToggle, registerToggle }) => {
  return (
    <header className="fixed top-0 inset-x-0 z-20 bg-black/40 backdrop-blur border-b border-white/10 text-white">
          ...
          {!user ? (
            <>
              ...
              <button
                ...
                className="rounded-xl px-4 py-2 text-sm md:text-base bg-white/10 text-white hover:bg-white/20 border border-white/10"
              >
                Log in
              </button>
              <button
